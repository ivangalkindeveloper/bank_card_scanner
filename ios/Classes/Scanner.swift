...
typealias ScannerCallback = ((UIImage, Error?) -> ())
typealias TorchCallback = ((Int?) -> ())
typealias ZoomCallback = ((Double?) -> ())

public class Scanner: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, FlutterTexture {
    ...
    let scannerCallback: ScannerCallback

    /// When torch mode is changes, this callback will be called
    let torchCallback: TorchCallback
    ...
    init(registry: FlutterTextureRegistry?, scannerCallback: @escaping ScannerCallback, torchCallback: @escaping TorchCallback, zoomCallback: @escaping ZoomCallback) {
        self.registry = registry
        self.scannerCallback = scannerCallback
        self.torchCallback = torchCallback
        self.zoomCallback = zoomCallback
        super.init()
    }

    func start(returnImage: Bool, cameraPosition: AVCaptureDevice.Position, torch: AVCaptureDevice.TorchMode, completion: @escaping (ScannerStartParameters) -> ()) throws {
        if (device != nil) {
            throw ScannerError.alreadyStarted
        }
        ...
        do {
            ...
            throw ScannerError.cameraError(error)
        }
    ...
    func stop() throws {
        if (device == nil) {
            throw ScannerError.alreadyStopped
        }
    ...
    func toggleTorch(_ torch: AVCaptureDevice.TorchMode) throws {
        if (device == nil) {
            throw ScannerError.torchWhenStopped
        }
        if (device.hasTorch && device.isTorchAvailable) {
            do {
                ...
                throw ScannerError.torchError(error)
            }
        }
    }
    ...
    func setScale(_ scale: CGFloat) throws {
        ...
        do {
            ...
            throw ScannerError.zoomError(error)
        }
    ...
    func resetScale() throws {
        if (device == nil) {
            throw ScannerError.zoomWhenStopped
        }
